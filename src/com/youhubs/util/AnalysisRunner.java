package com.youhubs.util;

import java.util.List;

import com.youhubs.BasicGridWorld;
...
public class AnalysisRunner {
	...
	public void runValueIteration(BasicGridWorld gen, Domain domain, State initialState, RewardFunction rf, TerminalFunction tf) {
		...
		for(int numIterations = increment;numIterations<=MAX_ITERATIONS;numIterations+=increment ){
			...
			//Added a very high delta number in order to guarantee that value iteration occurs the max number of iterations
			vi = new ValueIteration(domain,	rf,	tf,	0.99, new SimpleHashableStateFactory(), -1, numIterations); 
...
//		new EpisodeSequenceVisualizer(v, domain, Arrays.asList(ea));
	...
	public void runPolicyIteration(BasicGridWorld gen, Domain domain,
		...
		for(int numIterations = increment;numIterations<=MAX_ITERATIONS;numIterations+=increment ){
			long startTime = System.nanoTime();
			 pi = new PolicyIteration(domain,rf,tf,0.99,new SimpleHashableStateFactory(),-1, 1, numIterations);
...
//		new EpisodeSequenceVisualizer(v, domain, Arrays.asList(ea));
	...
	public void runQLearning(BasicGridWorld gen, Domain domain,
		...
		for(int numIterations = increment;numIterations<=MAX_ITERATIONS;numIterations+=increment ){
			...
			agent = new QLearning(domain, 0.99, new SimpleHashableStateFactory(), 0.99, 0.99);
