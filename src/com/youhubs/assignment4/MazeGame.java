package com.youhubs.assignment4;
...
public class MazeGame implements DomainGenerator {
	
	public static final String ATTX = "x";
	public static final String ATTY = "y";
	
	protected static final int X = 7; // Make different Maze Games
	protected static final int Y = 7; // Make different Maze Games
	protected static final int WALL = 30; // Make different Maze Games
/*	protected static final int X = 25; // Make different Maze Games
	protected static final int Y = 25; // Make different Maze Games
	protected static final int WALL = 15; // Make different Maze Games
*/	protected static final int SEED = 123;
	...
	protected int[][] getMap(int m, int n) {
		m = X; n = Y;
	    int[][] map = new int [m][n];
	    ...
	    if (map[m-1][n-2] == 1) map[m-2][n-1] = 0;
	    if (map[m-2][n-1] == 1) map[m-1][n-2] = 0;
	    map[m-1][n-1] = 0;
	...
	public static State getExampleState(Domain domain) {
		...
		location.setValue(ATTX, X-1);
		location.setValue(ATTY, Y-1);
	...
	protected class Movement extends SimpleAction implements FullActionModel {
			...
			for (int i = 0; i < this.directionProbs.length; i++) {
				...
				if (newPos[0] != curX || newPos[1] != curY) {
					...
					if (noChangeTransition != null) {
						...
						noChangeTransition = new TransitionProbability(
								s.copy(), this.directionProbs[i]);
		...
		protected int[] moveResult(int curX, int curY, int direction) {
			...
			int width = MazeGame.MAP.length;
			int height = MazeGame.MAP[0].length;

			// make sure new position is valid (not a wall or off bounds)
			if (nx < 0 || nx >= width || ny < 0 || ny >= height || MazeGame.MAP[nx][ny] == 1) {
				nx = curX;
				ny = curY;
			}
	...
	public class AgentPainter implements ObjectPainter {
		...
		public void paintObject(Graphics2D g2, State s, ObjectInstance ob, float cWidth, float cHeight) {
			...
			// set up floats for the width and height of our domain
			float fWidth = MazeGame.MAP.length;
			float fHeight = MazeGame.MAP[0].length;
